...
interface AreaFormProps {
  ...
  submitLabel?: string;
  onCancel: () => void;
  onSubmit: (payload: AreaFormSubmit) => void;
}
...
export function AreaForm({
  ...
  const [selected, setSelected] = useState<string[]>(initialCollaboratorIds);
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        onSubmit({ name, collaboratorIds: selected });
      }}
    >
      <Input onChange={(event) => setName(event.target.value)} placeholder="Nome da area" value={name} />

      <div className="space-y-2">
        <p className="text-xs uppercase tracking-widest text-flux-muted">Colaboradores da area</p>
